...
class Date
{
     ...
     */
    public static function find_date_uk( $string ) {
     ...
     * @return mixed  false if no date found else array: array( 'day' => 01, 'month' => 01, 'year' => 2012 )
     */
    public static function find_date_usa( $string ) {
      $shortenize = function( $string ) {
        return substr( $string, 0, 3 );
      };
      // Define month name:
      ...
      $month = "";
      $year = "";
      // Match dates: 12/24/2012 or 03-24-11 or 1 2 1985
      preg_match( '/([0-1]?[0-9])[\.\-\/ ]+([0-9]?[0-9])[\.\-\/ ]+([0-9]{2,4})/', $string, $matches );
      if ( $matches ) {
        if ( $matches[1] )
          $month = $matches[1];
        if ( $matches[2] )
